  ...
  return (
    <div className="space-y-5">
      ...
      <div className="glass relative overflow-hidden rounded-3xl p-6">
        ...
        <div className={cn("mt-4 inline-flex items-center gap-2 rounded-full border px-3 py-1 text-[13px]", onTrack ? "border-green/25 bg-green/5 text-green" : "border-amber/25 bg-amber/5 text-amber")}>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <ReviewList icon={<TrendingUp size={16} className="text-green" />} title="What moved" items={review.changes} tone="green" emptyLabel="Nothing yet — build today to get moving." />
        <ReviewList
      ...
      {/* next best move */}
      <div className="glass rounded-3xl border-cyan/15 p-6">
        <SectionLabel className="mb-2 text-cyan/80">Next best move</SectionLabel>
        ...
        <ul className="space-y-2">
          {items.map((it, i) => (
            <li key={i} className="flex items-start gap-2.5 text-[14px] text-ink/90">
              <span className={cn("mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full", tone === "green" ? "bg-green" : "bg-amber")} />
              {it}
            </li>
          ))}
        </ul>
