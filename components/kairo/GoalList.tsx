          ...
          {node.evidence && node.evidence.length > 0 && (
            <div className="mt-2.5 flex flex-col gap-1.5 border-t border-line pt-2.5">
              ...
              {node.evidence.map((ev) =>
                ev.kind === "link" ? (
                  <ExternalLink key={ev.id} href={ev.value.startsWith("http") ? ev.value : `https://${ev.value}`} className="truncate text-[13px] text-accent underline decoration-accent/30 underline-offset-2">{ev.value}</ExternalLink>
                ) : (
