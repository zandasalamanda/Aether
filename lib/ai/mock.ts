...
const TEMPLATES: Template[] = [
  {
    ...
    description: (t) =>
      `A focused build toward "${t}". Kairo mapped the path from a clear MVP to your first real users, ordered so momentum compounds.`,
  ...
  },
  {
    ...
    description: (t) =>
      `A calm study path toward "${t}". Kairo turned it into a rhythm you can actually hold, weighted toward your weak spots.`,
  ...
  },
  {
    ...
    description: (t) =>
      `A steady plan toward "${t}". Kairo broke it into small, repeatable moves so progress compounds without stress.`,
  ...
  },
  {
    ...
    description: (t) =>
      `A grounded path toward "${t}". Kairo started small and stackable so the routine sticks instead of stalling.`,
...
const DEFAULT_TEMPLATE: Template = {
  match: /.*/,
  description: (t) =>
    `A clear path toward "${t}". Kairo broke it into ordered steps so you always know the next move.`,
...
export function mockDailyPlan(input: DailyPlanInput): DailyPlanResult {
  ...
  const explanation =
    blocks.length === 0
      ...
      : input.energy === "low"
        ? "Energy is low, so Kairo kept blocks short and light — momentum matters more than volume today."
        : input.energy === "high"
          ? "Energy is high, so Kairo front-loaded the deepest work while you can carry it."
          : "Kairo balanced the day around what actually moves your goals, ordered by momentum.";
